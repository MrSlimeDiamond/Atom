    ...
    async execute(interaction) {
        if (interaction.options.getSubcommand() == 'cm') {
            ...
            const request = await axios.get(
                'https://board.portal2.sr/profile/' + username + '/json'
            )
            ...
            if (!data.profileNumber) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        'User ' +
                            username +
                            ' does not exist on board.portal.sr'
                    )
                ...
                let SPRank = data.points.SP.playerRank || 'Unranked'
                let CoopRank = data.points.COOP.playerRank || 'Unranked'
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        {
                            name: 'Singleplayer',
                            value: `Points: ${data.points.SP.score}\nRank: ${SPRank}`,
                            inline: true,
                        },
                        {
                            name: 'Cooperative',
                            value: `Points: ${data.points.COOP.score}\nRank: ${CoopRank}`,
                            inline: true,
                        },
                        {
                            name: 'Overall',
                            value: `Points: ${data.points.global.score}\nRank: ${OverallRank}`,
                            inline: true,
                        },

                        {
                            name: 'Best Rank',
                            value:
                                mapFromSteamID(data.times.bestRank.map) +
                                ' - ' +
                                data.times.bestRank.scoreData.playerRank,
                            inline: true,
                        },
                        {
                            name: 'Worst Rank',
                            value:
                                mapFromSteamID(data.times.worstRank.map) +
                                ' - ' +
                                data.times.worstRank.scoreData.playerRank,
                            inline: true,
                        },
                        {
                            name: 'World Records',
                            value: `Singleplayer: ${WRsSP}\nCooperative: ${WRsCOOP}\nOverall: ${WRs}`,
                            inline: true,
                        }
                    )
