...
class HelpCommand extends IRCCommand {
    ...
    onCommand(client, from, to, message) {
        ...
        if (args.length == 0) {
            ...
            this.sendIRCMessage(to, "Prefix: " + this.prefix, this.isHidden(message, from))
            ...
            if (!commands.includes(command_query)) {
                this.sendIRCMessage(to, "I don't recognize that command", this.isHidden(message, from))
                return
            }
            ...
            console.log(command)
            this.sendIRCMessage(to, command.name + " : " + command.description, this.isHidden(message, from))
        } else {
            this.sendIRCMessage(to, "No clue what you're tying to do", this.isHidden(message, from))
        }
