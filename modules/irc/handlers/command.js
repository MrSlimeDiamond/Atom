  ...
  handle(ircclient, from, to, message) {
    try {
      if (
        ...
        (message.split("> ")[1].startsWith("#" + this.prefix) &&
          from == "MCO_Telegram") ||
        (message.split(" ")[2].startsWith(this.prefix) && from == "McObot") ||
        (message.split(" ")[2].startsWith(this.prefix) && from == "Silly") ||
        (message.split(" ")[2].startsWith("#" + this.prefix) && from == "Silly")
      ) {
        ...
        if (!config.admins.includes(getCommandSender()) && command.adminonly) {
          logger.warn(
            `${getCommandSender()} tried using admin only command ${commandName}`
          );
      ...
      if (
        ...
        return;
      if (error == "TypeError: Cannot read property 'startsWith' of undefined")
        return;
      console.error(error);
    }
  }
