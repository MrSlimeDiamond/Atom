...
async function renderNext() {
  ...
  if (queue.length == 0 || rendering) {
    ...
    return
      ...
      let portal2LaunchScript = path.join(config.modules.portal2.portal2Path, "portal2.exe") + " -novid"
          ...
          (err) => {
            if (err) {
              const embed = new EmbedBuilder()
                ...
                .setDescription("The render completed, but the demo could not be renamed. If this persists, tell SlimeDiamond.")
            ...
            let videoURL = "http://" + config.modules.portal2.baseURL + "?user=" + message.author.id + "&id=" + renderNumber
            ...
            */
            ...
            message.channel.send("<@" + message.author.id + "> Your render is finished! URL: " + videoURL)
  ...
  client.on("messageCreate", async (message) => {
    ...
    if (message.content.startsWith("!render-demo")) {
      if (message.attachments.length < 0) {
        ...
        .setAuthor({ name: message.author.username, iconURL: message.author.avatarURL() })
        .setTitle("Render failed!")
        .setDescription("Please upload **1** Portal 2 demo file with the command.")
      message.reply({ embeds: [embed] })
        return
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription("Render was successfully queued at position " + queue.length + " . This can take a while. You will be notified when it is finished.")
