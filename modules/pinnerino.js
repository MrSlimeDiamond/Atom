  ...
  client.on("messageReactionAdd", async function (reaction, user) {
    ...
    if (reaction.partial) {
      try {
        ...
        logger.error(
          "Something went wrong when fetching message reactions: " + error
        );
        return;
      }
    }
    ...
    if (
      !config.guilds[message.guildId].reactions.includes(reaction._emoji.name)
    )
      return;
    ...
    async function pinMsg() {
      ...
      if (message.channel.id == config.guilds[message.guildId].channelID)
        return;
