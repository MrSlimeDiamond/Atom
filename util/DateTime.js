...
function DateTime (input, t = 'ms') {
    ...
    const data = {}
    , units = [{t:"ms",m:"1000"},{t:"seconds",m:60},{t:"minutes",m:60},{t:"hours",m:24},{t:"days",m:30},{t:"months",m:12},{t:"years",m:10}];
    
    units.forEach((unit) => input = (input - (data[unit.t] = (input % unit.m))) / unit.m);
    data.h = () => {
      return units.reverse()
      .filter((unit) => data[unit.t])
      .map((unit) => data[unit.t] + ' ' + (data[unit.t] === 1 ? unit.t.slice(0, -1) : unit.t))
      .join(', ') + ".";
    }
    return data;
  }
