...
public class CommandHandler extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        ...
        String commandName = event.getMessage().getContentRaw().split(prefix)[1].split(" ")[0]; // Not scuffed at all
        ...
        if (args.length > 0) {
            commands.forEach(cmd -> {
                ...
                if (name.equals(commandName) || aliases.contains(commandName)) {
                    ...
                    try {
                      ...
                      event.getMessage().replyEmbeds(EmbedUtil.genericIncorrectUsageEmbed(cmd.getCommand().usage())).queue();
                    } catch (Exception e) {
    ...
    public void executeCommand(AtomCommand command, MessageReceivedEvent msgEvent, SlashCommandInteractionEvent interaction) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException, IncorrectUsageException, SQLException {
